...
class TbHighCharts extends CWidget
{
	...
	{
		$assetsPath = Yii::getPathOfAlias('application.extensions.bootstrap.assets');
		$path = Yii::app()->assetManager->publish($assetsPath,true,-1,false);
		$path .= '/js/highcharts';

		Yii::app()->getClientScript()->registerScriptFile($path.'/highcharts.js');
		...
		if (isset($this->options['exporting']) && @$this->options['exporting']['enabled']) {
			Yii::app()->getClientScript()->registerScriptFile($path.'/modules/exporting.js');
		}
		if (isset($this->options['theme'])) {
			Yii::app()->getClientScript()->registerScriptFile($path.'/themes/'.$this->options['theme'].'.js');
		}
